...
export default function Navbar() {
  ...
  const isScreenSmall = useMediaQuery({ minWidth: 0, maxWidth: 1000 });
  const navbarItems = [
    {title:"/" }
  ]
      ...
      <>
        <div className=" mb-5">
          {["/courses", "/quiz", "/profile"].includes(location.pathname) ?
            <>
              <nav className={`navbar navbar-expand-lg fixed-top p-0 ${location.pathname === "/"  ? "bg-color text-white" : "bg-white"}`}>
                  ...
                  <div className={`collapse navbar-collapse ${["/courses", "/quiz", "/profile"].includes(location.pathname) ? "justify-content-around" : "justify-content-between"}`} id="navbarSupportedContent">
                    <ul className="navbar-nav">
                      <li className="nav-item">
                        <Link to={"/"} className={`nav-link text-black ${location.pathname === "/" ? "text-white" : ""}`} href="#">
                          Home
                        </Link>
                      </li>
                      <li className="nav-item">
            ...
            <>
              ...
              <nav className={`navbar navbar-expand-lg fixed-top p-0 ${location.pathname === '/' ? "bg-color text-white" : "bg-white"}`}>
                <div className="container">
                  <Link  className="navbar-brand " to={"/"}>
                    <img src={logo} className="rounded-5 " alt="logo" style={{ width: "89.62px", height: "px" }} />
                  </Link>
                  <button className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                    <span className="navbar-toggler-icon"></span>
                  </button>
                  <div className="collapse navbar-collapse justify-content-between" id="navbarSupportedContent">
                    ...
                    <div className={`${isScreenSmall ? "d-flex justify-content-center my-3" : ""}`}>
                      {location.pathname === "/library" ? (
                        ...
                        <>
                          <Link to={"/Login"} className={`btn mx-2 rounded-5 border-white border-2 text-color me-4 px-4 py-1 btn-sm ${location.pathname !== "/signup" ? "bg-color text-white" :  location.pathname === '/'  ? "text-white border-white" : ""}`} type="button">
                            Login
                          </Link>
                          <Link to={"/signup"} className={`btn rounded-5 text-color px-4 py-1 btn-sm main-border ${location.pathname === "/signup" ? "bg-color text-white" : location.pathname === '/'  ? "bg-white" : ""}`} type="button">
                            Sign Up
                          </Link>
                        </>
                      )}
